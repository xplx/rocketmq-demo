...
import com.aqlu.rocketmq.demo.domain.ProductWithPayload;
...
public class ProducerApplication implements CommandLineRunner{
    ...
    private void sendMessageMethod(){
        SendResult sendResult;
        // Send string
        sendResult = rocketMQTemplate.syncSend("spring-topic", "Hello, World!");
        System.out.printf("syncSend1 to topic %s sendResult=%s %n", "spring-topic", sendResult);

        sendResult = rocketMQTemplate.syncSend("user-topic", new User().setUserAge((byte) 18).setUserName("Kitty"));
        System.out.printf("syncSend1 to topic %s sendResult=%s %n", "user-topic", sendResult);
        ...
        // Use the extRocketMQTemplate,发送字符串，字节方式发送
        ...
        // Send user-defined object，使用异步方式，返回回调信息
